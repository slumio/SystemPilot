...
def generate_textbook():
    textbook_dir = "/home/joyboy/Systempilot/syspilot/docs/textbook"
    ...
    print("[*] Generating 10-Volume Master Architectural Textbook...")

    # Define 10 massive volumes with exhaustive technical depth
    volumes = []

    # Volume 1
    v1 = """# VOLUME 1: LINUX KERNEL INTERNALS, TRACING MECHANICS & eBPF CO-RE SUBSYSTEM
...
"""
    ...
    # Volume 2
    v2 = """# VOLUME 2: HARDWARE PMU PROFILING & NETLINK PROCESS TOPOLOGY
...
"""
    ...
    # Volume 3
    v3 = """# VOLUME 3: EDGE DAEMON ARCHITECTURE & COMPLETE C++ CLASS CATALOG
...
"""
    ...
    # Volume 4
    v4 = """# VOLUME 4: ZERO-ALLOCATION MEMORY ENGINE & CONCURRENCY SPECIFICATIONS
...
"""
    ...
    # Volume 5
    v5 = """# VOLUME 5: SIMD VECTORIZED EVENT PIPELINE & DEDUPLICATION
...
"""
    ...
    # Volume 6
    v6 = """# VOLUME 6: COLUMNAR TSDB STORAGE ENGINE & COMPRESSION MATH
...
"""
    ...
    # Volume 7
    v7 = """# VOLUME 7: INVERTED INDEXING & VECTORIZED QUERY ENGINE
...
"""
    ...
    # Volume 8
    v8 = """# VOLUME 8: MULTIMODAL CAUSAL REASONING MULTIGRAPH
...
"""
    ...
    # Volume 9
    v9 = """# VOLUME 9: MULTI-TIER IPC INFRASTRUCTURE & SHARED MEMORY RINGS
...
"""
    ...
    # Volume 10
    v10 = """# VOLUME 10: ENTERPRISE PRODUCTION SAFETY CONTROLS & RECOVERY PROTOCOLS
...
"""
    ...
    # Write each volume file
    for i, vol in enumerate(volumes, start=1):
        filename = f"VOLUME_{i}_SPECIFICATION.md"
        filepath = os.path.join(textbook_dir, filename)
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(vol)
        print(f"[+] Wrote Volume {i}: {filepath}")

    # Combine all volumes into a single master textbook
    master_textbook_path = os.path.join(docs_dir, "SYSPILOT_100_PAGE_SYSTEMS_TEXTBOOK.md")
    with open(master_textbook_path, "w", encoding="utf-8") as f:
        f.write("# SysPilot Master Systems Engineering Textbook\n\n")
        for vol in volumes:
            f.write(vol + "\n\n<div class=\"page-break\"></div>\n\n")

    print(f"[+] Combined Master Textbook Markdown created at: {master_textbook_path}")
    ...
    import re
    raw_md = open(master_textbook_path, "r", encoding="utf-8").read()
    body_html = re.sub(r'^# (.*?)$', r'<h1>\1</h1>', raw_md, flags=re.MULTILINE)
    body_html = re.sub(r'^## (.*?)$', r'<h2>\1</h2>', body_html, flags=re.MULTILINE)
    body_html = re.sub(r'^### (.*?)$', r'<h3>\1</h3>', body_html, flags=re.MULTILINE)

    html_file = os.path.join(docs_dir, "syspilot_100_page_master_textbook.html")
    pdf_docs = os.path.join(docs_dir, "syspilot_100_page_master_textbook.pdf")
    pdf_artifact = os.path.join(artifacts_dir, "syspilot_100_page_master_textbook.pdf")

    html_doc = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>SysPilot Master Systems Engineering Textbook</title>
    {css}
</head>
<body>
<div class="cover-page">
    <div class="cover-title">SysPilot Systems Engineering Textbook</div>
    <div class="cover-subtitle">10-Volume Master Specification Manual for Production Linux Observability</div>
    <p><strong>Kernel eBPF Probes · Daemon Class Catalog · Columnar TSDB · Causal Graph Engine</strong></p>
    <div style="margin-top: 80px; font-size: 10pt; color: #475569;">
        <p><strong>Document Ref:</strong> SIOP-TEXTBOOK-2026-V10</p>
        <p><strong>Status:</strong> Complete 10-Volume Engineering Specification</p>
    </div>
</div>
{body_html}
</body>
...
"""
    ...
    print(f"[+] Master HTML textbook written to: {html_file}")

    print("[*] Compiling Master Textbook PDF via LibreOffice...")
    cmd = [
    ...
    if res.returncode == 0:
        print(f"[+] Successfully rendered PDF: {pdf_docs}")
        subprocess.run(["cp", pdf_docs, pdf_artifact], check=True)
        print(f"[+] Successfully copied PDF artifact to: {pdf_artifact}")
    else:
