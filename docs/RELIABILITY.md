...
2. The included [systemd service](../deploy/syspilot.service) restarts the daemon after a crash.

Install the release binary and enable the service:

```bash
sudo install -m 0755 target/release/syspilot /usr/local/bin/syspilot
sudo install -m 0644 deploy/syspilot.service /etc/systemd/system/syspilot.service
sudo systemctl daemon-reload
sudo systemctl enable --now syspilot
```

Verify both layers with `syspilot status`, `systemctl status syspilot`, and `cat /tmp/syspilot-health.json`. Production host monitoring should alert on a heartbeat older than three seconds and failed service restarts.
